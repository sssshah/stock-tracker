...
- `NSApp.presentationOptions` (hide menu bar) is blocked by App Sandbox — cannot reclaim menu bar space in maximized mode. Hard Apple restriction, no workaround for sandboxed apps. Gap between macOS menu bar and ticker in maximized mode is accepted behavior.
- `onChange(of:)` with single-parameter closure is deprecated in macOS 14 — use two-parameter form `{ _, newValue in }`
- Console message `Unable to obtain a task name port right for pid 596: (os/kern) failure (0x5)` is a harmless AppIntents/sandbox system message — not an app error, safe to ignore
- Yahoo Finance v8/chart endpoint available as price data fallback but has CORS issues in browsers — works from native Mac context only

## News Misattribution Log
*Examples collected for future news logic refinement session:*

| URL | Mapped To | Should Be | Root Cause |
|---|---|---|---|
| benzinga.com — Nasdaq-100 rallies, Intel jumps, Iran ceasefire | Portfolio symbol | Nobody | Broad market recap, no symbol specificity. Also resurfaced from previous day. |
| app.moby.co — airline industry drowning in record revenue | NVDA | Nobody | Pure Finnhub tagging error — no Nvidia relevance |
| cnbc.com — Apple kicks off WWDC 2026 | MSFT | AAPL | Cross-symbol steal — fixed in current version via headline prominence check |
| finance.yahoo.com — Oracle stock sinking as AI spending climbs | GOOGL | Nobody/ORCL | Competitor/sector bleed — Oracle tagged alongside GOOGL by Finnhub |
| finance.yahoo.com — Intel stock jumps BofA surprise | Portfolio symbol | Nobody/INTL | Intel not in portfolio but tagged to a portfolio symbol by Finnhub |

*Patterns to address in future session: broad market recaps, competitor sector bleed, stale story resurfacing*
